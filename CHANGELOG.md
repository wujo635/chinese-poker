...
### Added
- The app version now shows on Home, at the bottom of the page (e.g. "v0.22.1"). `vite.config.ts` reads `package.json`'s version at build time and injects it as a `__APP_VERSION__` global (declared in new `src/vite-env.d.ts`), so it's always in sync with the version being shipped — no manual string to keep updated separately.
